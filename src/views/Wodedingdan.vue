    ...
    <div class="tab-box">
      ...
      >
        <van-tab :title="v.title" v-for="(v, i) in tabItems" :key="i">
          <div v-if="orderDatas.length > 0">
            <div class="tab-item" v-for="(item, index) in orderDatas" :key="index">
              <div class="clearfix tab-item-box">
                ...
                <div class="fr">
                  <div class="text" v-if="item.status == 1" @click="queren(item, index)">确认收货</div>
                ...
                <div>
                  <div class="clearfix cell-box" v-for="(value, i) in item.products" :key="i">
                    <div class="fl pro-img">
                      <img class="img_auto" :src="value.smallImg" alt />
                    </div>
          ...
          <div v-else>
            <van-empty description="订单数据空空如也~">
              <van-button round color="#0C34BA" class="bottom-button" @click="goCaidan">去逛一逛~</van-button>
            </van-empty>
          </div>
        </van-tab>
      </van-tabs>
...
<script>
import "../assets/less/wodedingdan.less";
